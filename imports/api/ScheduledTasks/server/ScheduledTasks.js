...
if (Meteor.isServer) {
    SyncedCron.add({
        ...
        job: function () {
            ...
            let user = Meteor.users.findOne({"profile.head" : true});
            let athletes = [];
            let k = 0;
            for(let i = 0; i < user.profile.teamAccess.length; i++){
                if(user.profile.teamAccess[i] !== undefined){
                    let athletes2 = Athletes.find( { "teamId" : user.profile.teamAccess[i] }).fetch();
