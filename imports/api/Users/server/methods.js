...
Meteor.methods({
    /*Definition for users.addNewRole (Server Side Method), will be called by client who will pass through attributes:
        * @Params targetId, roles ; This function will add a new role to a user within the users collection.*/
    ...
    * @Params updateInfo ; This function will change the role of a particular user within the users collection.*/
  'users.changeRole': function usersChangeRole(updateInfo) {
    ...
    * @Params targetUserId, roles, group ; This function will update the role of a particular user within the users collection.*/
  'users.updateRoles': function usersUpdateRoles (targetUserId, roles, group=null) {
    ...
    * @Params updateInfo ; This function will send a verification email to user with the corresponding userId within the users collection.*/
  ...
  'users.update': function usersUpdate(user_obj) {
      ...
      Meteor.users.update(user_obj.id, {
        $set: { 'emails.0.address': user_obj.email, 'profile.name.first': user_obj.firstName, 'profile.name.last': user_obj.lastName }
      });
  ...
  'users.createNew_WithPswd': function usersCreateNewWithPswd(user_obj) {
    ...
    const user_info = { email: user_obj.email, password: user_obj.password };
    // on server: returns id if on client(see Registration.jsx): logs in newly created user, has callback
    const id = Accounts.createUser(user_info);
    Meteor.users.update({_id: id}, {
      $set: { profile: { name: { first: user_obj.name, last: user_obj.lastName } } }
    });
  ...
  'users.createNew_NoPswd': function usersCreateNewNoPswd(user_obj) {
    ...
    const user_info = { email: user_obj.email };
    const id = Accounts.createUser(user_info);
    Meteor.users.update({_id: id}, {
      $set: { profile: { name: { first: user_obj.name, last: user_obj.lastName } } }
    });
    ...
    * @Params userID ; This function will remove the user's account with the corresponding id passed through. */
